...
const providerDefaults: Record<string, AiProviderRuntimeConfig> = {
  deepseek: {
    apiKey: '',
 ...
 * 获取提供方默认配置
 */
export function getAiProviderDefaults(providerId: AiProviderId): AiProviderRuntimeConfig {
...
export function resolveAiProviderConfig(
  ...
  const defaults = getAiProviderDefaults(resolvedProviderId)
  if (!providerConfig && !defaults.baseUrl && !defaults.defaultModel) {
    throw new Error(`AI 配置缺失：未找到提供方 ${resolvedProviderId}`)
  }
  const apiKey = clientOverride.apiKey || providerConfig?.apiKey || defaults.apiKey
  const baseUrl = clientOverride.baseUrl || providerConfig?.baseUrl || defaults.baseUrl
  const defaultModel = clientOverride.defaultModel || providerConfig?.defaultModel || defaults.defaultModel
  if (!apiKey) {
