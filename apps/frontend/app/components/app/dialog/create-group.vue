...
const loadFriends = async () => {
    ...
    try {
        ...
        if (res.success && res.data) {
            friends.value = res.data.items.filter(user => 
                    ...
                    <main class="flex-1 overflow-y-auto p-8 pb-6">
                        ...
                        <div v-if="step === 1" class="space-y-8">
                            ...
                            <div class="flex flex-col items-center space-y-4">
                                <div class="relative group">
                                    <div class="avatar">
                                        <div class="w-24 h-24 rounded-2xl border-4 border-base-200 shadow-lg transition-all group-hover:shadow-xl">
                                    ...
                                    <!-- 悬浮效果 -->
                                    <div v-if="!groupAvatar" class="absolute inset-0 rounded-2xl bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer" @click="triggerAvatarUpload"></div>
                                </div>
                        ...
                        <div v-if="step === 2" class="space-y-6">
                            ...
                            <div v-if="selectedFriends.length" class="space-y-3">
                                <div class="flex items-center justify-between">
                                    <h3 class="text-sm font-medium text-base-content/70">已选择 {{ selectedFriends.length }} 位成员</h3>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    >
                                        ...
                                        <Icon name="mingcute:close-line" size="12" class="cursor-pointer" @click="toggleMember(friend.id)" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="space-y-2 max-h-80 overflow-y-auto custom-scrollbar">
                                    ...
                                    <div v-if="friendsLoading" class="space-y-3">
                                        <div v-for="i in 5" :key="i" class="flex items-center gap-4 p-3 rounded-xl bg-base-200/30 animate-pulse">
                                    ...
                                    <div v-else-if="filteredFriends.length === 0" class="text-center py-12">
                                        <Icon name="mingcute:user-search-line" size="48" class="text-base-content/20 mx-auto mb-4" />
                                        <p class="text-base-content/40 text-sm">{{ searchQuery ? '没有找到相关好友' : '暂无好友' }}</p>
                                    </div>
                                    ...
                                    <div v-else class="space-y-1">
                                        ...
                                        >
                                            <!-- 头像 -->
                                            <div class="avatar">
                                                <div class="w-12 h-12 rounded-full border-2 border-transparent group-hover:border-primary/20 transition-all">
                                                    <img v-if="friend.image" :src="friend.image" :alt="friend.name" class="object-cover" />
                                                    <div v-else class="w-full h-full bg-gradient-to-br from-primary/10 to-primary/5 flex items-center justify-center">
                                                        <span class="text-primary font-medium">{{ getInitial(friend) }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div class="font-medium text-base-content truncate">{{ getDisplayName(friend) }}</div>
                    ...
                    <footer class="p-8 pt-6 border-t border-base-200/50">
                        <div class="flex items-center gap-3">
                            ...
                            <button 
                                ...
                                :disabled="!groupName.trim() || groupName.trim().length < 2"
                                @click="nextStep"
                            >
                                下一步
