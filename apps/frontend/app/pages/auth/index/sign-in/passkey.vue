  ...
  <div class="w-full max-w-sm mx-auto">
    <div class="flex items-center gap-2 mb-2">
      <div class="text-3xl inline-flex items-center gap-2">
        <span class="font-bold font-siyuan">Passkey 验证</span>
    ...
    <div class="flex flex-col gap-6">
      <div ref="contentContainer" class="flex flex-col items-center gap-4">
        <div class="relative w-24 h-24">
          ...
          <svg v-if="loading" ref="loadingCircle" class="absolute inset-0 w-24 h-24 text-primary/30" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
      ...
      <div ref="buttonContainer" class="flex flex-col gap-3">
        <button type="button" class="btn btn-primary btn-block" :disabled="loading" @click="handlePasskeyVerify">
        ...
        <button type="button" class="btn btn-ghost btn-block" :disabled="loading" @click="handleCancel">取消</button>
      </div>
...
const statusDescription = ref('点击下方按钮开始 Passkey 验证')
...
function showError(message: string) {
  ...
  statusDescription.value = '请检查您的 Passkey 设置'
...
async function handlePasskeyVerify() {
  ...
  if (!isPasskeyAvailable()) {
    showError('当前环境不支持 Passkey。HTTP + IP 地址不支持，请改用 localhost 或 HTTPS 域名。')
    return
  }
  ...
  try {
    const result = await useAuthClient().signIn.passkey({
      fetchOptions: {
        onResponse(context) {
          ...
          if (token) userStore.setAuthToken(token)
        },
      },
    })
    ...
    if (result.error) {
      ...
      showError(result.error.message || 'Passkey 验证失败，请重试')
      return
    }

    const sessionToken = (result.data as any)?.session?.token
    if (sessionToken) userStore.setAuthToken(sessionToken)
    ...
    stopLoadingAnimation()
    showError(error?.message || 'Passkey 验证失败，请重试')
  } finally {
    loading.value = false
  }
}
...
function handleCancel() {
  errorMsg.value = ''
  if (!contentContainer.value || !buttonContainer.value) {
    router.back()
    return
  }
