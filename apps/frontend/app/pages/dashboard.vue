...
const featureGroups = [
    {
        ...
        items: [
            ...
            },
            {
                ...
                name: '群组',
                description: '创建和管理群聊',
                icon: 'mingcute:people-team-line',
                iconBg: 'bg-info/10',
                iconColor: 'text-info',
                path: '/groups',
                category: 'communication'
                ...
                <div class="space-y-8">
                    <section v-for="group in featureGroups" :key="group.category" class="space-y-4">
                        <div class="flex items-center gap-2">
                            <div class="flex items-center justify-center w-8 h-8 rounded-xl bg-base-200">
                        ...
                        <div class="grid gap-3 sm:gap-4" :class="getGridCols()">
                            <div v-for="item in group.items" :key="item.id" @click="handleFeatureClick(item)"
                                class="group relative flex flex-col rounded-2xl border border-base-200 bg-base-100 p-4 cursor-pointer transition-all duration-300 hover:border-base-300 hover:bg-base-200/50 hover:shadow-lg hover:shadow-base-content/5 hover:-translate-y-0.5 active:scale-[0.98] sm:rounded-3xl sm:p-5"
                                :class="getCardPadding()">
                                <div class="flex items-start justify-between">
                                    <div class="flex items-center gap-3 sm:gap-4">
                                        <div class="flex shrink-0 items-center justify-center rounded-xl transition-transform duration-300 group-hover:scale-110"
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <p class="mt-0.5 truncate transition-colors"
                                    ...
                                    <div v-if="item.badge" class="flex shrink-0">
                                        <span
                                            class="badge badge-sm badge-circle"
                                            :class="[item.badgeColor || 'badge-primary']">
                                            {{ item.badge }}
                                        </span>
                                    </div>
                                </div>

                                <div class="absolute inset-x-4 bottom-4 flex items-center justify-end opacity-0 transition-all duration-300 group-hover:opacity-100 sm:inset-x-5">
                                    <div
                                        class="flex items-center gap-1 rounded-full bg-base-content/5 px-2 py-1 text-xs font-medium text-base-content/60">
                                        <span>前往</span>
