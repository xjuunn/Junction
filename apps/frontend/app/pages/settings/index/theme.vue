...
function handleResetTheme() {
    ...
    themeSettings.compactMode = false;
    themeSettings.animationsEnabled = true;
    themeSettings.animationSpeed = 'normal';
            ...
            <div class="space-y-8 max-w-2xl">
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <button v-for="t in themes" :key="t.value" @click="themeSettings.theme = t.value"
                            class="card p-4 border-2 transition-all cursor-pointer hover:scale-[1.02]"
                            :class="themeSettings.theme === t.value ? 'border-primary bg-primary/5' : 'border-base-300'">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 gap-3">
                        <button v-for="c in primaryColors" :key="c.value" @click="themeSettings.primaryColor = c.value"
                            class="flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all cursor-pointer hover:scale-[1.05]"
                            :class="themeSettings.primaryColor === c.value ? 'border-primary bg-primary/5' : 'border-base-300'">
                    ...
                    <div class="space-y-4">
                        <div class="form-control">
                            <label class="label cursor-pointer justify-start gap-4">
                                <input type="checkbox" v-model="themeSettings.compactMode"
                                    class="toggle toggle-primary" />
                                <span class="label-text">
                                    <span class="font-bold">紧凑模式</span>
                                    <br />
                                    <span class="text-sm text-base-content/50">减少间距，使界面显示更多内容</span>
                                </span>
                            </label>
                        </div>
                        ...
                        <div class="form-control">
                            <label class="label cursor-pointer justify-start gap-4">
                                <input type="checkbox" v-model="themeSettings.sidebarCollapsed"
                                    class="toggle toggle-primary" />
                                <span class="label-text">
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        <div class="form-control">
                            <label class="label cursor-pointer justify-start gap-4">
                                <input type="checkbox" v-model="themeSettings.animationsEnabled"
                                    class="toggle toggle-primary" />
                                <span class="label-text font-bold">启用动画效果</span>
                            </label>
                        </div>

                        <div class="form-control w-full" v-if="themeSettings.animationsEnabled">
                            <label class="label">
                                <span class="label-text font-bold">动画速度</span>
                            </label>
                            <div class="flex flex-wrap gap-2">
                                <button v-for="s in animationSpeeds" :key="s.value"
                                    @click="themeSettings.animationSpeed = s.value"
                                    class="btn btn-sm rounded-lg"
                                    :class="themeSettings.animationSpeed === s.value ? 'btn-primary' : 'btn-ghost'">
                                    {{ s.label }}
                                </button>
                            </div>
                        </div>
                ...
                <div class="card bg-base-200/50 p-4 rounded-xl">
                    <div class="flex items-center justify-between">
                        ...
                        <button class="btn btn-sm btn-ghost" @click="handleResetTheme">
                            重置
                        </button>
                    </div>
                </div>
