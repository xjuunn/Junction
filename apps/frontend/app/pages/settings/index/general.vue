...
import { DOWNLOAD_PATH_KEY, setSavedDownloadDir } from '~/utils/download';
...
const isResolvingDownloadDir = ref(false);
const isSelectingDir = ref(false);

onMounted(() => {
  if (typeof window === 'undefined') return;
  downloadPath.value = window.localStorage.getItem(DOWNLOAD_PATH_KEY) || '';
});
...
const resolveSystemDownloadDir = async () => {
  if (!isTauriEnv || isResolvingDownloadDir.value) return;
  isResolvingDownloadDir.value = true;
  try {
    const { downloadDir } = await import('@tauri-apps/api/path');
    const dir = await downloadDir();
    if (dir) downloadPath.value = dir;
  } catch (err: any) {
    toast.error(err?.message || '获取系统下载目录失败');
  } finally {
    isResolvingDownloadDir.value = false;
  }
};
...
const selectDownloadDir = async () => {
  ...
  try {
    ...
    if (typeof result === 'string' && result.trim()) {
      downloadPath.value = result.trim();
    }
  } catch (err: any) {
      ...
      <div class="space-y-8 max-w-2xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="form-control w-full">
            ...
            <select v-model="formData.dateFormat" class="select select-bordered w-full focus:select-primary bg-base-100">
          ...
          <div class="form-control w-full">
            ...
            <select v-model="formData.timeFormat" class="select select-bordered w-full focus:select-primary bg-base-100">
        ...
        <div v-if="isTauriEnv" class="rounded-2xl border border-base-200 bg-base-100/60 p-4 md:p-5">
          <div class="flex flex-col sm:flex-row sm:items-start gap-4">
            <div class="flex-1 space-y-2">
              ...
              <input v-model="downloadPath" type="text" class="input input-bordered w-full bg-base-100"
            ...
            <div class="flex gap-2 sm:flex-col">
              ...
              <button class="btn btn-ghost btn-sm" @click="downloadPath = ''">清空</button>
            </div>
          </div>
        </div>
