...
export interface McServerClientHandle {
  ...
  ) => Promise<Array<{ id?: string; name?: string; permissionLevel?: number; bypassesPlayerLimit?: boolean }>>
  ...
  ) => Promise<Array<{ id?: string; name?: string; reason?: string | null; source?: string | null; expires?: string | null }>>
...
export interface McServerCreateInput {
  id?: string
  name: string
...
export type McServerUpdateInput = Partial<{
  name: string
  protocol: 'mc-management'
  url: string
  token: string
